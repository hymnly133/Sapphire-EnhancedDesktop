...
EDToolTip::EDToolTip(QWidget *parent)
    : QWidget{parent}
{
    ...
    connect(this,&EDToolTip::nowAlpha_changed,this,[=](int val){
        whenAnimationChange();
    });
    connect(this,&EDToolTip::nowRadius_changed,this,[=](int val){
        whenAnimationChange();
    });
    connect(this,&EDToolTip::nowSize_changed,this,[=](QSize val){
        whenAnimationChange();
    });
    connect(this,&EDToolTip::nowPos_changed,this,[=](QPoint val){
        whenAnimationChange();
    });
    // connect(animations,&)
...
void EDToolTip::setInfo(QString info)
{
    this->info = info;
...
void EDToolTip::end()
{
    ...
    connect(animations,&QParallelAnimationGroup::finished,this,&EDToolTip::whenEndAnimationEnd);

    animations->start();
}

void EDToolTip::Tip(QString info)
{
    Tip(pls->mapFromGlobal(QCursor::pos()),info);
}

void EDToolTip::Tip(QPoint pos, QString info)
{
    //ls pos
    EDToolTip* Tip = new EDToolTip(pls);
    Tip->move(pos+QPoint(13,8));
    Tip->setInfo(info);
    Tip->comeout();
}

void EDToolTip::Tip(QWidget *aim, QPoint pos, QString info)
{
    Tip(mapToLS(aim,pos),info);
}

void EDToolTip::whenAnimationChange()
{
...
void EDToolTip::whenEndAnimationEnd()
{
    deleteLater();
}

void EDToolTip::paintEvent(QPaintEvent *event)
{
...
void EDToolTip::resizeEvent(QResizeEvent *event)
{
    rs->updateDisplay();
