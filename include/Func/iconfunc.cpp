...
void IconReader::scanForDefault()
{
    ...
    if(!QFile::exists(iconDirPath)) return;
...
{
    QString null;
    if(!Exe2Package.contains(exeName)) return null;
    QString package = Exe2Package.values(exeName).first();
    if(!Package2Icon.contains(package)) return null;
    QString icon = Package2Icon.values(package).first();
    if(!icons.contains(icon)) return null;
    return icons[icon];
}
