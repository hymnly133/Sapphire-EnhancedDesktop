...
void SMenu::addShellCommand(QPair<QString, QString> command)
{
    ...
    if(exitsCommands.contains(command.first)) return;
...
{
    if(!added){
        SAction* syscomAct = new SAction(tr("更多"));
        addAction(syscomAct);
...
{
    ignoreCommands<<"find"<<"UpdateEncryptionSettings"<<"explore"<<"open"<<"opennewprocess"<<"opennewtab"<<"opennewwindow"<<"pintohome";
    readDirReg(REG_Dir);
    dirCommands.append(QPair("S",""));
    dirBGCommands.append(QPair("S",""));
