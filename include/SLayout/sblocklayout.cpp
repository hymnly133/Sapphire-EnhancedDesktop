...
SBlockLayout::SBlockLayout(SLayoutContainer *father, int row, int col, double boradXR, double boradYR, double spaceXR, double spaceYR): SLayout(father)
{
    this->row = row;
    this->col = col;
...
QSize SBlockLayout::unit2Size(SUnit* aim)
{
...
QPoint SBlockLayout::unit2Pos(SUnit* aim)
{
    return QPoint(blocks[aim->indX][aim->indY]->posX(), blocks[aim->indX][aim->indY]->posY());
}
